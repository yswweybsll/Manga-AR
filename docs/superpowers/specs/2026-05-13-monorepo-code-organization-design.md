...
顶层只承载 workspace 管理、通用配置、文档、仓库级脚本和单一共享包 `shared/`。具体客户端和服务运行时代码放进 `apps/`。
...
```text
apps/mobile          -> shared
apps/studio-desktop  -> shared
apps/relay           -> shared
shared               -> 无平台依赖
```

`shared` 不依赖 React Native、Expo、Electron、Three.js、Node WebSocket 或任何平台运行时。它只放平台无关的类型、纯函数、常量和 schema。
...
Three.js 场景对象、TransformControls、桌面 UI 状态都留在此包内，不进入 `shared`。
...
从当前 `src/types/*` 中抽平台无关类型到 `shared/src/`。mobile 改为从 `@manga-ar/shared` 引用共享类型。relay 也开始引用共享同步消息类型，减少协议漂移。
